...
@router.put('/add-content-to-playlist', status_code=status.HTTP_201_CREATED,response_model=PlayListResponse)
def add_content_to_playlist(playlist_data: AddContentToPlaylist, db: Session = Depends(get_db),
                            current_user: User = Depends(get_current_user)):
    playlist_query = db.query(models.PlayList).filter(models.PlayList.id == playlist_data.playlist_id)
